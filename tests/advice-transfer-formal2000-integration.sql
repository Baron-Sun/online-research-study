...
do $test$
declare
  cohort text:='6ab3fa4fe3086041cbc44a37';
  previous text:='6ab145947e9c974c2b6cf1cd';
  prefix text:='formal2000-rehearsal-'||gen_random_uuid()::text;
...
begin
  ...
  select md5(jsonb_agg(to_jsonb(s) order by id)::text) into historical_submissions
    from public.advice_transfer_submissions s where study_id<>cohort and not is_test;
  select md5(jsonb_agg(to_jsonb(a) order by id)::text) into historical_assignments
    from public.advice_transfer_assignments a where study_id<>cohort and not is_test;
  select md5(jsonb_agg(to_jsonb(q) order by id)::text) into historical_tokens
    from public.advice_transfer_quota_tokens q where study_id<>cohort;
  ...
  error_message:=null;
  begin perform public.claim_advice_transfer_assignment_ratings(prefix||'-wrong-study','wrong-study','new',false,null,null);
  ...
  error_message:=null;
  begin perform public.claim_advice_transfer_assignment_ratings(old_pid,cohort,'new',false,null,null);
  exception when others then get stacked diagnostics error_message=message_text; end;
  ...
  -- Allocate 20 distinct new participants through the production entry point.
  for i in 1..20 loop
    pid:=prefix||'-'||i;
    claim:=public.claim_advice_transfer_assignment_ratings(pid,cohort,'new',false,null,null);
    if claim->>'admissionStatus' is distinct from 'assigned'
      ...
      or claim->>'ratingScaleVersion' is distinct from 'ratings-0-100-v1'
  ...
  if (select count(*) from public.advice_transfer_assignments where study_id=cohort and not is_test and reservation_kind='quota')<>20
    or (select count(distinct quota_token_id) from public.advice_transfer_assignments where study_id=cohort and not is_test)<>20
    or exists(select 1 from public.advice_transfer_formal_cell_progress
      where reserved<>1 or token_total<>100 or available<>99 or not quota_invariant_ok) then
    raise exception '20 entrants did not create one unique reservation in each of 20 cells'; end if;

  -- A withdrawal must be replaced in the one cell that now has fewer reservations.
  select * into displaced from public.advice_transfer_assignments where study_id=cohort
    and not is_test and reservation_kind='quota' order by id limit 1;
  perform public.withdraw_advice_transfer_assignment(displaced.assignment_id,displaced.prolific_pid,'participant_withdrew');
  pid:=prefix||'-replacement';
  claim:=public.claim_advice_transfer_assignment_ratings(pid,cohort,'new',false,null,null);
  select * into standby from public.advice_transfer_assignments where assignment_id=claim->>'assignmentId';
  if claim->>'admissionStatus' is distinct from 'assigned' or standby.reservation_kind is distinct from 'quota'
    or standby.stimulus_id is distinct from displaced.stimulus_id or standby.condition is distinct from displaced.condition then
    raise exception 'Replacement did not restore the underfilled cell'; end if;
  ...
  for assignment in select distinct on(stimulus_id,condition) * from public.advice_transfer_assignments
    ...
    phase1:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','ratingScaleVersion','ratings-0-100-v1','commentJudgments',judgments,
      'gistText',repeat('word ',25),'gistDifficulty',0,'perceptionQuestionsVersion','perception-questions-v2-reasons',
    ...
    again:=public.claim_advice_transfer_assignment_ratings(pid,cohort,'new',false,null,null);
    ...
    phase2:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','ratingScaleVersion','ratings-0-100-v1','adviceText',repeat('word ',77),
      'difficulty',100,'effort',null,'confidence',0,'timings',jsonb_build_object('adviceResponseTimeMs',16000));
    perform public.save_advice_transfer_stage_perception(assignment.assignment_id,pid,'phase2',phase2);
    payload:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','ratingScaleVersion','ratings-0-100-v1','assignmentId',assignment.assignment_id,
      ...
      'commentsStoodOutDetails','','aiGeneratedBelief','unsure','aiLikelihood',100,
    ...
    if saved.study_id<>cohort or saved.material_version<>'content-priority-v2-20260921'
      or saved.rating_scale_version<>'ratings-0-100-v1' or saved.gist_difficulty<>0 or saved.difficulty<>100 or saved.confidence<>0 or saved.ai_likelihood<>100
  ...
  if checked<>100 or (select count(*) from public.advice_transfer_submissions where study_id=cohort and not is_test)<>20
    or exists(select 1 from public.advice_transfer_formal_cell_progress where pending<>1 or reserved<>0 or available<>99 or not quota_invariant_ok) then
    raise exception 'Post-submission balance failed'; end if;

  if (select md5(jsonb_agg(to_jsonb(s) order by id)::text) from public.advice_transfer_submissions s where study_id<>cohort and not is_test)
      is distinct from historical_submissions
    or (select md5(jsonb_agg(to_jsonb(a) order by id)::text) from public.advice_transfer_assignments a where study_id<>cohort and not is_test)
      is distinct from historical_assignments
    or (select md5(jsonb_agg(to_jsonb(q) order by id)::text) from public.advice_transfer_quota_tokens q where study_id<>cohort)
      is distinct from historical_tokens then raise exception 'Historical cohort was modified'; end if;
end; $test$;
rollback;
select 'PASS: 2000 available slots checked; 20 allocated participants across 20 balanced cells; same-cell replacement, old-participant exclusion, 100 comment hashes, all-wrong-label continuation, saves, repeat submissions and historical-data preservation; synthetic records rolled back' as result;
