...
do $test$
declare
  prefix text:='qa-ratings-'||gen_random_uuid()::text;
...
begin
  ...
  for row in select * from public.advice_transfer_stimuli where active and pair_role='primary' and pair_number in (1,13) order by pair_number loop
    foreach condition_name in array array['human','ai'] loop
      pid:=prefix||'-'||row.pair_number||'-'||condition_name;
      claim:=public.claim_advice_transfer_assignment_ratings(pid,'revised-qa','new',true,row.pair_number,condition_name);
      if claim->>'ratingScaleVersion'<>'ratings-0-100-v1' or claim->>'materialVersion'<>'content-priority-v2-20260921'
      ...
      phase1:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','commentJudgments',judgments,
        'gistText',repeat('word ',25),'ratingScaleVersion','ratings-0-100-v1','gistDifficulty',0,'perceptionQuestionsVersion','perception-questions-v2-reasons',
        'perceivedConsensus',0,'roomForDisagreement',100,
        'timings',jsonb_build_object('phase1ActiveTimeMs',12000,'gistActiveTimeMs',4000));
      foreach actual in array array['-1','101','2.5','null','"4"','true'] loop
        bad:=jsonb_set(phase1,'{gistDifficulty}',actual::jsonb); err:=null;
        ...
        if err is null then raise exception 'Invalid gist rating accepted: %',actual; end if;
      end loop;
      bad:=phase1-'ratingScaleVersion'; err:=null;
      begin perform public.save_advice_transfer_stage_perception(claim->>'assignmentId',pid,'phase1',bad);
      exception when others then get stacked diagnostics err=message_text; end;
      if err is null then raise exception 'Missing scale version accepted'; end if;
      ...
      again:=public.claim_advice_transfer_assignment_ratings(pid,'revised-qa','new',true,row.pair_number,condition_name);
      if again->>'ratingScaleVersion'<>'ratings-0-100-v1' or again#>>'{phase1Snapshot,gistDifficulty}'<>'0' or again->'comments'<>claim->'comments' or again->'phase1Snapshot'<>result->'phase1Snapshot' then
      ...
      phase2:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','adviceText',repeat('word ',77),
        'ratingScaleVersion','ratings-0-100-v1','difficulty',100,'effort',null,'confidence',0,'timings',jsonb_build_object('adviceResponseTimeMs',16000));
      bad:=jsonb_set(phase2,'{confidence}','101'); err:=null;
      begin perform public.save_advice_transfer_stage_perception(claim->>'assignmentId',pid,'phase2',bad);
      exception when others then get stacked diagnostics err=message_text; end;
      if err is null then raise exception 'Invalid phase 2 rating accepted'; end if;
      perform public.save_advice_transfer_stage_perception(claim->>'assignmentId',pid,'phase2',phase2);
      result:=public.save_advice_transfer_stage_perception(claim->>'assignmentId',pid,'phase2',phase2);
      if result->>'alreadySaved'<>'true' then raise exception 'Stage retry not idempotent'; end if;
      payload:=jsonb_build_object('schemaVersion','advice-transfer-v4-gist','assignmentId',claim->>'assignmentId',
        ...
        'aiGeneratedBelief','unsure','ratingScaleVersion','ratings-0-100-v1','aiLikelihood',case when condition_name='ai' then 100 else 0 end,'materialVersion','forged','perceivedConsensus',999,
      ...
      foreach actual in array array['-1','101','2.5','null','"4"','true'] loop
        bad:=jsonb_set(payload,'{aiLikelihood}',actual::jsonb); err:=null;
        begin perform public.submit_advice_transfer_payload(claim->>'assignmentId',bad);
        exception when others then get stacked diagnostics err=message_text; end;
        if err is null then raise exception 'Invalid source rating accepted: %',actual; end if;
      end loop;
      perform public.submit_advice_transfer_payload(claim->>'assignmentId',payload);
      result:=public.submit_advice_transfer_payload(claim->>'assignmentId',payload);
      if result->>'alreadySubmitted'<>'true' then raise exception 'Final retry not idempotent'; end if;
      select * into saved from public.advice_transfer_submissions where assignment_id=claim->>'assignmentId';
      if saved.rating_scale_version<>'ratings-0-100-v1' or saved.gist_difficulty<>0 or saved.difficulty<>100 or saved.confidence<>0
        or saved.ai_likelihood<>(case when condition_name='ai' then 100 else 0 end)
        or saved.full_payload#>>'{serverAudit,ratingScaleVersion}'<>'ratings-0-100-v1'
        or saved.material_version<>'content-priority-v2-20260921'
  ...
  if checked<>20 then raise exception 'Not all test comment exposures checked'; end if;
...
rollback;
select 'PASS: 0 and 100 preserved, invalid values rejected, refresh and retry safe, incorrect comment labels allowed; tests rolled back' as result;
